...
app.get("/userGames/:steamId",  async (req, res) => {
    ...
    let userData = await axios.get("http://api.steampowered.com/ISteamUser/GetPlayerSummaries/v0002/?key=" + apiKey + "&steamids=" + req.params.steamId + "&format=json")
    ...
    if (userData.data.response.players[0].communityvisibilitystate == 3) {
        ...
        //Showing Owned Games
        let games = await axios.get("http://api.steampowered.com/IPlayerService/GetOwnedGames/v0001/?key=" + apiKey + "&steamid=" + req.params.steamId + "&format=json&include_appinfo=1&include_played_free_games=1")
            let total = 0
            for (let game of games.data.response.games) {
                let gameData = await axios.get("https://api.isthereanydeal.com/v02/game/plain/?key=2a2fc17c80b93372f570423e56c4eadc04d36934&shop=steam&title=" + game.name)
                let cost = await axios.get("https://api.isthereanydeal.com/v01/game/prices/?key=2a2fc17c80b93372f570423e56c4eadc04d36934&plains=" + gameData.data.data.plain)
            ...
            for (let game of games.data.response.games) {
                ...
                gamesString = gamesString.concat(" , ")
            }
