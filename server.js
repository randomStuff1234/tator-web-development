    ...
    axios.get("http://api.steampowered.com/ISteamUser/GetPlayerSummaries/v0002/?key=" + apiKey + "&steamids=" + req.params.steamId + "&format=json")
        .then((userData) => {
            ...
            if (userData.data.response.players[0].communityvisibilitystate == 3) {
                ...
                //Showing Owned Games
                axios.get("http://api.steampowered.com/IPlayerService/GetOwnedGames/v0001/?key=" + apiKey + "&steamid=" + req.params.steamId + "&format=json&include_appinfo=1&include_played_free_games=1").then ((gamesInfo) => {
                    let total = 0
                    ...
                    const getGameCost = (i) => {
                        if (i == games.length) {
                            ...
                            res.send("Total: " + total + " Games: " + gamesString.slice(0, gamesString.length - 2));
                            return;
                        }
                        console.log("test123")
                        let { name } = games[i];
